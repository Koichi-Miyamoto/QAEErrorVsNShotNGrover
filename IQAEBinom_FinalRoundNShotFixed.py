...
def IQAEBinom_FinalRoundNShotFixed(ampSq, epsilon, alpha, nShotUnit, nShotFinal, confint_method="chernoff", minRatio=2):
    ...
    while True:
        ...
        if not(isFinalRound): # final roundならrRoundを前ラウンドより持ち越し
            rRound = int(kRound * thetaInterval[0] / (0.5 * np.pi))

        while not(goNextRound):
            if isFinalRound:
                nShotRound = nShotFinal
            else:
